...
> Note: extension modules declared via `package.json` `omp.extensions` **are** loaded from marketplace installs — installation symlinks the cached plugin into the scope's `node_modules` and records it in `ohmg-plugins.lock.json`, the same runtime surfaces used by npm-installed and `omp plugin link`ed plugins.
