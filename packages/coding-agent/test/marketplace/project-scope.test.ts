...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .omp/ directory", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".ohmg"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".ohmg", "plugins", "installed_plugins.json"));
	});
	...
	it("walk-up stops at the nearest .omp/ — does not skip to a more distant one", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".ohmg"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".ohmg"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".ohmg", "plugins", "installed_plugins.json"));
	});
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".ohmg", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tmpProject, ".ohmg", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".ohmg", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".ohmg", "plugins", "installed_plugins.json");
	});
