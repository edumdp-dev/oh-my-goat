...
describe("PluginManager.install load validation", () => {
	...
	test("rejects an install whose extension entry cannot resolve its dependencies", async () => {
		...
		expect(await Bun.file(path.join(tmpRoot, "ohmg-plugins.lock.json")).exists()).toBe(false);
	});
	...
	test("restores the previous package tree when reinstall validation fails", async () => {
		...
		await Bun.write(
			path.join(tmpRoot, "ohmg-plugins.lock.json"),
			JSON.stringify(
		...
		const lock = await Bun.file(path.join(tmpRoot, "ohmg-plugins.lock.json")).json();
