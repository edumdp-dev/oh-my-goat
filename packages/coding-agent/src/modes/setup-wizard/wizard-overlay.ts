...
import { DISPLAY_NAME } from "@oh-my-pi/pi-utils";
import { GOAT_LOGO, GOAT_WORDMARK, gradientLogo } from "../components/welcome";
...
export class SetupWizardComponent implements Component, OverlayFocusOwner {
	...
	#renderScene(width: number, height: number): string[] {
		...
		const logo = height >= GOAT_LOGO.length + 18 ? gradientLogo(GOAT_LOGO, 0) : [theme.bold(GOAT_WORDMARK)];
		const header = [
			"",
			...logo.map(line => centerLine(line, width)),
			centerLine(theme.bold(theme.fg("accent", DISPLAY_NAME)), width),
